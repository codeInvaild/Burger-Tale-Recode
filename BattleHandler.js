...
export let battleUI = {//this purely handles ui which is drawn over everything,
    ...
    draw(){
        ...
        if (this.state.action === "player") {
            ...
            if (currentMenuName === "main") {
                ...
                for (let i = 0; i < mainUIIndexes.length; i++) {
                    ...
                    let label = newText("mainMenuLabel"+i, rect.x + 10, currentAlly.worldData.y + 85, "rgb(0,255,255)", "16px JetBrains Mono ExtraBold", mainUIIndexes[i]);
                    label.draw();
                }
                ...
                newText("desc_battle",(gameWidth/2) -400 + 20,(gameHeight/2)-100,"rgb(255,255,255)","30px JetBrains Mono ExtraBold", battlefield.allies[this.state.allyIndex].actions[this.state.menuStack[this.state.menuStack.length-1].curIndex]).draw();
                newText("desc_battle",(gameWidth/2) -400 + 20,(gameHeight/2)-60,"rgb(255,255,255)","30px JetBrains Mono ExtraBold", "Energy Cost: "+ playerActionDirectory[battlefield.allies[this.state.allyIndex].actions[this.state.menuStack[this.state.menuStack.length-1].curIndex]].cost).draw();
                newWrappedText("desc_battle",(gameWidth/2) -400 + 20,(gameHeight/2)-20,760,"rgb(255,255,255)","30px JetBrains Mono ExtraBold", playerActionDirectory[battlefield.allies[this.state.allyIndex].actions[this.state.menuStack[this.state.menuStack.length-1].curIndex]].description).draw();
                ...
                let tt = newText("jw",currentAlly.worldData.x + 250,currentAlly.worldData.y + 50 + 16,"rgb(255,255,255)","16px JetBrains Mono ExtraBold","yo target someone twin");
                tt.draw();
        ...
        if (this.errorMessage) {
            ...
            newWrappedText("errorMessage",dialogueBoxX,900,dialogueBoxWidth,"rgb(255,255,255)","36px JetBrains Mono ExtraBold",this.errorMessage).draw();
        }
        ...
        newFilledWrappedText("dialogueBoxText",dialogueBoxX + 10,dialogueBoxY + dialogueBoxTextSize + 10,dialogueBoxWidth,"rgb(255,255,255)",dialogueBoxTextSize+"px JetBrains Mono ExtraBold",this.battleBoxText,25,"left",true).draw();
...
export let battle = {
    ...
    clear() {
        ...
        battlefield.previousState = battleStates.INACTIVE;
        battlefield.state = battleStates.INACTIVE;
        battlefield.stateInfo = {};//metadata to prevent massive enums
        battlefield.itemIndexBuffer=[];//keep index of items to delete next cycle (this was really annoying to deal with)


        battleUI.renderQueue = [];
        battleUI.battleBoxText = ""
        battleUI.errorMessage=""
        battleUI.itemsInUse=[];
        battleUI.state = {
            targetType : "none",
                action : "none",
    ...
    draw(){
        ...
        for (let ai=0;ai<battlefield.allies.length; ai++) {
            ...
            newFilledText("allyHPText_"+ai,ally.worldData.x,ally.worldData.y+ally.worldData.height+15,"rgb(255,255,255)","20px JetBrains Mono ExtraBold",ally.health.current+"/"+ally.health.max,false,"rgb(0,0,0)",3).draw();
        }
        ...
        if (battlefield.state === battleStates.PLAYER_SELECTION) {
            for (let ai=0;ai<battlefield.allies.length; ai++) {
                let ally = battlefield.allies[ai];
                newFilledText("allyHPText_"+ai,ally.worldData.x,ally.worldData.y+ally.worldData.height+45,"rgb(255,255,255)","20px JetBrains Mono ExtraBold","Energy: "+ (ally.selectedMoveInstance?(ally.energy.current - ally.selectedMoveInstance.cost+"/"+ally.energy.max) :(ally.energy.current+"/"+ally.energy.max) ),false,"rgb(0,0,0)",3).draw();
            }
        }
