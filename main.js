...
function loop() {//MAIN GAME HAPPENS HERE
    ...
    if (gameState === "Start") { //we want to handle specific update functions accordingly to each game state
        ...
        shopHandler.update(deltaTime);
    } else if (gameState === "Death") {

    }
    dialogue.update(deltaTime);
    draw();
    let fps = newText("fps_counter",0,60,"rgb(255,255,255)","60px JetBrains Mono ExtraBold","FPS: "+ Math.ceil(1/deltaTime) );
    fps.draw()
...
let allAssets = Object.keys(assets.images).length + Object.keys(assets.music).length +Object.keys(assets.sounds).length +Object.keys(assets.fonts).length;

console.log(allAssets);

for (let fnt in assets.fonts) {
    loadAsset("fonts", assets.fonts[fnt], fnt).then(() => {
