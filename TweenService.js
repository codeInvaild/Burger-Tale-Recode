...
class Tween {
    ...
    constructor(object, tweenInfo, goal) {
        ...
        for (let key in goal) {
            ...
            if (this.properties[key].origin === null || this.properties[key].end === null) {
                throw new Error("You have a NULL value in the tween here");
            }
        }


        // --- NEW: newest tween wins, property by property ---
        const incomingKeys = Object.keys(this.properties);
        for (let i = Tween.allTweens.length - 1; i >= 0; i--) {
            const other = Tween.allTweens[i];
            if (other === this || other.object !== object) continue;

            for (const key of incomingKeys) {
                if (key in other.properties) {
                    delete other.properties[key]; // this older tween no longer owns that property
                }
            }

            // if the older tween has nothing left to animate, retire it
