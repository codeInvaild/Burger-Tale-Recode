        ...
        if (type === "images") {
            ...
            font.load()
                .then((loadedFont) => {
                    ...
                    assetCount++;
                    availableAssets[type][id] = loadedFont;
                    console.log(`Font loaded: ${id}`);
                    resolve(loadedFont);
                })
                .catch((err) => reject("Failed to load font " + id + ": " + err));
        }
    });
}
...
// }
//
// await loadFont("JetBrains Mono ExtraBold", "Assets/fonts/JetBrainsMono-ExtraBold.ttf");