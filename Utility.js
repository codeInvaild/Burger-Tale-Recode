...
export function newFilledWrappedText(id, x, y, maxWidth, color = "rgb(255,255,255)", font = "30px JetBrains Mono ExtraBold", text = "hello!", lineHeight = 25, align = "left", ignoreCamera = false, outlineColor = "black", outlineWidth = 2) {
    renderedShapes[id] = {X: x, Y: y, MaxWidth: maxWidth, Color: color, Font: font, Text: text, LineHeight: lineHeight, Align: align, OutlineColor: outlineColor, OutlineWidth: outlineWidth, draw: function () {
            ...
            if (!ignoreCamera && cameraBehavior.curCam === "battle") {
                drawX = (s.X - battleCamera.x) * battleCamera.zoom;
                drawY = (s.Y - battleCamera.y) * battleCamera.zoom;
            }
            ...
            lines.forEach((line, i) => {
                const lineY = drawY + (i * s.LineHeight);
                for (const [ox, oy] of offsets) {
                    ctx.fillText(line, drawX + ox, lineY + oy);
                }
            });

            // real text pass, on top
            ctx.fillStyle = s.Color;
            lines.forEach((line, i) => {
                ctx.fillText(line, drawX, drawY + (i * s.LineHeight));
            });
        }};
    return renderedShapes[id];
}
...
export function newFilledText(id, x, y, color = "rgb(255,255,255)", font = "30px JetBrains Mono ExtraBold", text = "hello!", ignoreCamera = false, outlineColor = "black", outlineWidth = 2) {
    renderedShapes[id] = {X: x, Y: y, Color: color, Font: font, Text: text, OutlineColor: outlineColor, OutlineWidth: outlineWidth, draw: function () {
            ...
            // draw the outline as 8 offset copies in a ring around the real position
            const w = s.OutlineWidth;
            const offsets = [
                [-w, -w], [0, -w], [w, -w],
                [-w,  0],          [w,  0],
            ...
            for (const [ox, oy] of offsets) {
                ctx.fillText(s.Text, drawX + ox, drawY + oy);
            }

            // real text on top, covering the center of the outline ring
            ctx.fillStyle = s.Color;
            ctx.fillText(s.Text, drawX, drawY);
        }}
    return renderedShapes[id];
}
...
export class damageCounter {
    ...
    static draw() {
        for (let counterIndex = 0; counterIndex < damageCounter.allCounters.length; counterIndex++) {
            let curCount = damageCounter.allCounters[counterIndex];
            newFilledText(curCount.name,curCount.x,curCount.y,curCount.color+curCount.colorAlpha+")","36px JetBrains Mono ExtraBold",curCount.amount).draw();
